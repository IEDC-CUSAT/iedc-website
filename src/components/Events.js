...
const data = [
    {
        ...
        description: "Gain insider insights, guidance from experts, and network with peers. Don't miss this opportunity to kickstart your entrepreneurial journey!",
        icon: Event1,
        date: "March 20, 2024",
...
const Card = ({ heading, icon, date, index, description, venue, time }) => {
    ...
    useEffect(() => {
        ...
        if (!isMobile) {
            ...
            gsap.set(`.landing #text-${index}`, { x: "-30%", duration: 0.3, width: "137%", opacity: 1, transformOrigin: "right bottom" });
        }
    }, [index, isMobile]);
    ...
    const handleHoverExit = () => {
        ...
        if (!isMobile) {
            ...
            gsap.to(`.landing #text-${index}`, { x: "-30%", duration: 0.3, width: "137%", opacity: 1, transformOrigin: "right bottom" });
        }
    };

    return (
        <motion.div className='overflow-hidden flex w-full odd:border-y-[2px] border-opacity-40 border-black gap-2 md:gap-4 py-5 md:py-8 cursor-pointer' key={index}
            onMouseEnter={handleHover}
            onMouseLeave={handleHoverExit}
        >
            <div className=' grid grid-cols-12 flex-1 landing '>
                <img id={`img-${index}`} src={icon} alt="icon" className='col-span-12 md:col-span-4 w-full md:w-11/12 my-auto block rounded-2xl' />
                <div id={`text-${index}`} className='col-span-12 md:col-span-8 flex flex-col justify-center md:justify-between gap-y-5 md:gap-y-3'>
                    <div className='gap-y-2 flex flex-col'>
                        ...
                        <p className=' font-normal text-xl md:text-3xl'>{description}</p>
